...
> **Важно про формы.** Доставка заявок на почту уже реализована в коде
> (`app/api/contact/route.ts`). Чтобы письма приходили — задайте SMTP-доступ
> вашего ящика в `.env.local` (см. **Шаг 11**). Пока SMTP не настроен, заявки
> дублируются в лог сервера (`pm2 logs`), поэтому не теряются.
